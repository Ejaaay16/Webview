...
$sqlins1 = "INSERT INTO `auto`(`email`, `fpath`, `fname`) VALUES ('$emadd','$mfile','$fname')";
$stmtins1 = mysqli_prepare($link, $sqlins1);
mysqli_stmt_execute($stmtins1);
mysqli_stmt_close($stmtins1);
unset($_POST);
unset($_FILES);
header("location:teacher_auto.php?lgacc=" . $lgd_acc);
}
//function getF($link, $lgd_acc){
    $result = $link -> query("SELECT * FROM `auto` WHERE `email`= '" . $lgd_acc . "'");
                ...
                <tbody>
                    ...
                    <tr>
                        <td data-label="Upload"  style="color:black !important;">
                            <form action="<?php echo "teacher_auto.php?lgacc=" . $lgd_acc;?>" method="post" enctype="multipart/form-data">
