...
$sqlins1 = "INSERT INTO `he_exam`(`email`, `link`) VALUES ('$emadd','$lnk')";
$stmtins1 = mysqli_prepare($link, $sqlins1);
mysqli_stmt_execute($stmtins1);
mysqli_stmt_close($stmtins1);
unset($_POST);
unset($_FILES);
header("location:teacher_he_exam.php?lgacc=" . $lgd_acc);
}
//function getF($link, $lgd_acc){
    $result = $link -> query("SELECT * FROM he_exam WHERE `email`= '" . $lgd_acc . "'");
...
<head> 
  ...
  <link rel="stylesheet" type="text/css" href="css/teacher_he_exam.css">
...
<body>
    ...
    <a href="<?php echo "logout.php?lgacc=" . $lgd_acc;?>" class="Logout_btn">Logout</a>
      ...
      <form name="add_link" id="add_link" action="<?php echo "teacher_he_exam.php?lgacc=" . $lgd_acc;?>" method = "post">  
