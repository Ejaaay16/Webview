...
$sqlins1 = "INSERT INTO `gen2`(`email`, `fpath`, `fname`) VALUES ('$emadd','$mfile','$fname')";
$stmtins1 = mysqli_prepare($link, $sqlins1);
mysqli_stmt_execute($stmtins1);
mysqli_stmt_close($stmtins1);
unset($_POST);
unset($_FILES);
header("location:teacher_gen2.php?lgacc=" . $lgd_acc);
}
//function getF($link, $lgd_acc){
    $result = $link -> query("SELECT * FROM gen2 WHERE `email`= '" . $lgd_acc . "'");
...
<head> 
  ...
  <title>Reading in Philippine History</title>  
                ...
                <tbody>
                    ...
                    <tr>
                        <td data-label="Upload"  style="color:black !important;">
                            <form action="<?php echo "teacher_gen2.php?lgacc=" . $lgd_acc;?>" method="post" enctype="multipart/form-data">
